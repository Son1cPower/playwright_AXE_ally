...
test.describe('Playwright web page accessibility test', async () => {
    ...
    test(`For Devices list`, async () => {
        ...
        const devices = await page.locator("a[class='testio-nav-item hidden-xs-down'] span[class='icon icon-devices mr-0']")
        await devices.click();
        const mesages_chat = await page.locator("div[aria-label='Open Intercom Messenger']")
        expect(await mesages_chat).to.exist
