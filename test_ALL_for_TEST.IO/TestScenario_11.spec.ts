...
test.describe('Playwright web page accessibility test', async () => {
    ...
    test(`Settings > Locale options`, async () => {
        ...
        const localeOptions = await page.locator("a[href='/account/settings']")
        await localeOptions.click();


        url = await page.url()
        expect(url).to.equal('https://tester.test.io/account/settings')
